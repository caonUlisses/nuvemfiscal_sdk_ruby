  ...
  class NfcomSefazProd
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NuvemfiscalSdkRuby::NfcomSefazProd` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NuvemfiscalSdkRuby::NfcomSefazProd`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @c_class.to_s.length < 7
        invalid_properties.push('invalid value for "c_class", the character length must be great than or equal to 7.')
      end
    ...
    def c_class=(c_class)
      ...
      if !c_class.nil? && c_class.to_s.length < 7
        fail ArgumentError, 'invalid value for "c_class", the character length must be great than or equal to 7.'
      end
