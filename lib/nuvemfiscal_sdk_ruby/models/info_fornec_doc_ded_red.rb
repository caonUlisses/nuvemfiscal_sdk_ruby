  ...
  class InfoFornecDocDedRed
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NuvemfiscalSdkRuby::InfoFornecDocDedRed` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NuvemfiscalSdkRuby::InfoFornecDocDedRed`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def nif=(nif)
      ...
      if !nif.nil? && nif.to_s.length < 1
        fail ArgumentError, 'invalid value for "nif", the character length must be great than or equal to 1.'
      end
    ...
    def hash
      [cnpj, cpf, nif, c_nao_nif, caepf, im, x_nome, _end, fone, email].hash
    end
