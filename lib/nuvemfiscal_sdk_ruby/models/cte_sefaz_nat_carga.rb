  ...
  class CteSefazNatCarga
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NuvemfiscalSdkRuby::CteSefazNatCarga` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NuvemfiscalSdkRuby::CteSefazNatCarga`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@x_dime.nil? && @x_dime.to_s.length < 5
        invalid_properties.push('invalid value for "x_dime", the character length must be great than or equal to 5.')
      end
    ...
    def x_dime=(x_dime)
      ...
      if !x_dime.nil? && x_dime.to_s.length < 5
        fail ArgumentError, 'invalid value for "x_dime", the character length must be great than or equal to 5.'
      end
