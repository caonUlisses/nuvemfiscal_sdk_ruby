  ...
  class NfcomSefazDest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NuvemfiscalSdkRuby::NfcomSefazDest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NuvemfiscalSdkRuby::NfcomSefazDest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@id_outros.nil? && @id_outros.to_s.length < 2
        invalid_properties.push('invalid value for "id_outros", the character length must be great than or equal to 2.')
      end
    ...
    def id_outros=(id_outros)
      ...
      if !id_outros.nil? && id_outros.to_s.length < 2
        fail ArgumentError, 'invalid value for "id_outros", the character length must be great than or equal to 2.'
      end
