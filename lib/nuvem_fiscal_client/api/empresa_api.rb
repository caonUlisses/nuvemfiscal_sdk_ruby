...
module NuvemFiscalClient
  class EmpresaApi
    ...
    # @option opts [Integer] :expires_in Filtrar certificados que expiram dentro de X dias.    Informe um número inteiro correspondente à quantidade de dias até o vencimento.  Exemplos:   - expires_in&#x3D;30 -&amp;gt; certificados que vencem nos próximos 30 dias.   - expires_in&#x3D;7  -&amp;gt; certificados que vencem nos próximos 7 dias.
    # @option opts [Boolean] :include_expired Indicar se os certificados já vencidos devem ser incluídos no resultado.    Valores aceitos:   - &#x60;true&#x60;: incluir certificados vencidos.   - &#x60;false&#x60;: exibir apenas certificados válidos. (default to true)
    # @return [EmpresaCertificadoListagem]
    def listar_certificados(opts = {})
      data, _status_code, _headers = listar_certificados_with_http_info(opts)
      data
    end
    ...
    # @option opts [Integer] :expires_in Filtrar certificados que expiram dentro de X dias.    Informe um número inteiro correspondente à quantidade de dias até o vencimento.  Exemplos:   - expires_in&#x3D;30 -&amp;gt; certificados que vencem nos próximos 30 dias.   - expires_in&#x3D;7  -&amp;gt; certificados que vencem nos próximos 7 dias.
    # @option opts [Boolean] :include_expired Indicar se os certificados já vencidos devem ser incluídos no resultado.    Valores aceitos:   - &#x60;true&#x60;: incluir certificados vencidos.   - &#x60;false&#x60;: exibir apenas certificados válidos. (default to true)
    # @return [Array<(EmpresaCertificadoListagem, Integer, Hash)>] EmpresaCertificadoListagem data, response status code and response headers
    def listar_certificados_with_http_info(opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: EmpresaApi.listar_certificados ...'
      end
      if @api_client.config.client_side_validation && !opts[:'expires_in'].nil? && opts[:'expires_in'] <= 0
        fail ArgumentError, 'invalid value for "opts[:"expires_in"]" when calling EmpresaApi.listar_certificados, must be greater than 0.'
      end

      # resource path
      local_var_path = '/empresas/certificados'
      ...
      query_params[:'expires_in'] = opts[:'expires_in'] if !opts[:'expires_in'].nil?
      query_params[:'include_expired'] = opts[:'include_expired'] if !opts[:'include_expired'].nil?
      ...
      # return_type
      return_type = opts[:debug_return_type] || 'EmpresaCertificadoListagem'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['jwt', 'oauth2']

      new_options = opts.merge(
        :operation => :"EmpresaApi.listar_certificados",
        :header_params => header_params,
        :query_params => query_params,
        :form_params => form_params,
        :body => post_body,
      ...
      data, status_code, headers = @api_client.call_api(:GET, local_var_path, new_options)
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: EmpresaApi#listar_certificados\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end
    ...
    # @option opts [String] :nome_razao_social Filtrar pelo nome ou razão social da empresa.    Esse filtro realiza uma correspondência pelo início do texto,  retornando apenas empresas cujo nome ou razão social começam com  o valor informado.    *Caso o filtro pelo CPF ou CNPJ também seja informado na requisição,  este filtro é ignorado*.
    ...
    def listar_empresas_with_http_info(opts = {})
      ...
      query_params[:'nome_razao_social'] = opts[:'nome_razao_social'] if !opts[:'nome_razao_social'].nil?
