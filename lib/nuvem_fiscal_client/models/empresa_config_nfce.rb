  ...
  class EmpresaConfigNfce
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NuvemFiscalClient::EmpresaConfigNfce` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NuvemFiscalClient::EmpresaConfigNfce`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'sefaz')
        self.sefaz = attributes[:'sefaz']
      else
        self.sefaz = nil
      end
    ...
    def list_invalid_properties
      ...
      if @sefaz.nil?
        invalid_properties.push('invalid value for "sefaz", sefaz cannot be nil.')
      end
    ...
    # @param [Object] sefaz Value to be assigned
    def sefaz=(sefaz)
      if sefaz.nil?
        fail ArgumentError, 'sefaz cannot be nil'
      end
