  ...
  class EmpresaConfigRps
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NuvemFiscalClient::EmpresaConfigRps` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NuvemFiscalClient::EmpresaConfigRps`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'lote')
        self.lote = attributes[:'lote']
      else
        self.lote = nil
      end
    ...
    def list_invalid_properties
      ...
      if @lote.nil?
        invalid_properties.push('invalid value for "lote", lote cannot be nil.')
      end
      ...
      if @serie.to_s.length > 20
        invalid_properties.push('invalid value for "serie", the character length must be smaller than or equal to 20.')
      end
    ...
    def valid?
      ...
      return false if @lote.nil?
      return false if @serie.nil?
      return false if @serie.to_s.length > 20
      return false if @numero.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] lote Value to be assigned
    def lote=(lote)
      if lote.nil?
        fail ArgumentError, 'lote cannot be nil'
      end
    ...
    def serie=(serie)
      ...
      if serie.to_s.length > 20
        fail ArgumentError, 'invalid value for "serie", the character length must be smaller than or equal to 20.'
      end
