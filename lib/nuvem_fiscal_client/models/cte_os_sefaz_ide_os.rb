  ...
  class CteOsSefazIdeOS
    ...
    def self.openapi_types
      {
        ...
        :'x_mun_fim' => :'String',
        :'uf_fim' => :'String',
        :'inf_percurso' => :'Array<CteOsSefazInfPercursoOS>',
        :'dh_cont' => :'Time',
        :'x_just' => :'String',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NuvemFiscalClient::CteOsSefazIdeOS` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NuvemFiscalClient::CteOsSefazIdeOS`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      return false if !@x_mun_ini.nil? && @x_mun_ini.to_s.length > 60
      return false if !@x_mun_ini.nil? && @x_mun_ini.to_s.length < 2
      return false if !@x_mun_fim.nil? && @x_mun_fim.to_s.length > 60
      return false if !@x_mun_fim.nil? && @x_mun_fim.to_s.length < 2
    ...
    def hash
      [c_uf, c_ct, cfop, nat_op, mod, serie, n_ct, dh_emi, tp_imp, tp_emis, c_dv, tp_amb, tp_cte, proc_emi, ver_proc, c_mun_env, x_mun_env, uf_env, modal, tp_serv, ind_ie_toma, c_mun_ini, x_mun_ini, uf_ini, c_mun_fim, x_mun_fim, uf_fim, inf_percurso, dh_cont, x_just, g_compra_gov].hash
    end
