  ...
  class VDescCondIncond
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NuvemFiscalClient::VDescCondIncond` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NuvemFiscalClient::VDescCondIncond`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@v_desc_incond.nil? && @v_desc_incond < 0
        invalid_properties.push('invalid value for "v_desc_incond", must be greater than or equal to 0.')
      end

      if !@v_desc_cond.nil? && @v_desc_cond < 0
        invalid_properties.push('invalid value for "v_desc_cond", must be greater than or equal to 0.')
      end
    ...
    def v_desc_incond=(v_desc_incond)
      if !v_desc_incond.nil? && v_desc_incond < 0
        fail ArgumentError, 'invalid value for "v_desc_incond", must be greater than or equal to 0.'
      end
    ...
    def v_desc_cond=(v_desc_cond)
      if !v_desc_cond.nil? && v_desc_cond < 0
        fail ArgumentError, 'invalid value for "v_desc_cond", must be greater than or equal to 0.'
      end
    ...
    def hash
      [v_desc_incond, v_desc_cond].hash
    end
