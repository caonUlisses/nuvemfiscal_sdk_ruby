  ...
  class NfeSefazICMS15
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NuvemFiscalClient::NfeSefazICMS15` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NuvemFiscalClient::NfeSefazICMS15`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'ad_rem_icms_reten')
        self.ad_rem_icms_reten = attributes[:'ad_rem_icms_reten']
      else
        self.ad_rem_icms_reten = nil
      end

      if attributes.key?(:'v_icms_mono_reten')
        self.v_icms_mono_reten = attributes[:'v_icms_mono_reten']
      else
        self.v_icms_mono_reten = nil
      end

      if attributes.key?(:'p_red_ad_rem')
        self.p_red_ad_rem = attributes[:'p_red_ad_rem']
      end

      if attributes.key?(:'mot_red_ad_rem')
        self.mot_red_ad_rem = attributes[:'mot_red_ad_rem']
      end
    end
    ...
    def list_invalid_properties
      ...
      if @ad_rem_icms_reten < 0
        invalid_properties.push('invalid value for "ad_rem_icms_reten", must be greater than or equal to 0.')
      end

      if @v_icms_mono_reten < 0
        invalid_properties.push('invalid value for "v_icms_mono_reten", must be greater than or equal to 0.')
      end

      if !@p_red_ad_rem.nil? && @p_red_ad_rem > 1E+2
        invalid_properties.push('invalid value for "p_red_ad_rem", must be smaller than or equal to 1E+2.')
      end

      if !@p_red_ad_rem.nil? && @p_red_ad_rem < 0
        invalid_properties.push('invalid value for "p_red_ad_rem", must be greater than or equal to 0.')
      end
    ...
    def valid?
      ...
      return false if @ad_rem_icms < 0
      return false if @v_icms_mono < 0
      return false if !@q_bc_mono_reten.nil? && @q_bc_mono_reten < 0
      return false if @ad_rem_icms_reten < 0
      return false if @v_icms_mono_reten < 0
      return false if !@p_red_ad_rem.nil? && @p_red_ad_rem > 1E+2
      return false if !@p_red_ad_rem.nil? && @p_red_ad_rem < 0
      true
    end
    ...
    # Custom attribute writer method with validation
    # @param [Object] ad_rem_icms_reten Value to be assigned
    def ad_rem_icms_reten=(ad_rem_icms_reten)
      if !ad_rem_icms_reten.nil? && ad_rem_icms_reten < 0
        fail ArgumentError, 'invalid value for "ad_rem_icms_reten", must be greater than or equal to 0.'
      end

      @ad_rem_icms_reten = ad_rem_icms_reten
    end
    ...
    # Custom attribute writer method with validation
    # @param [Object] p_red_ad_rem Value to be assigned
    def p_red_ad_rem=(p_red_ad_rem)
      if !p_red_ad_rem.nil? && p_red_ad_rem > 1E+2
        fail ArgumentError, 'invalid value for "p_red_ad_rem", must be smaller than or equal to 1E+2.'
      end

      if !p_red_ad_rem.nil? && p_red_ad_rem < 0
        fail ArgumentError, 'invalid value for "p_red_ad_rem", must be greater than or equal to 0.'
      end

      @p_red_ad_rem = p_red_ad_rem
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          ad_rem_icms_reten == o.ad_rem_icms_reten &&
          v_icms_mono_reten == o.v_icms_mono_reten &&
          p_red_ad_rem == o.p_red_ad_rem &&
          mot_red_ad_rem == o.mot_red_ad_rem
    end
    ...
    def hash
      [orig, cst, q_bc_mono, ad_rem_icms, v_icms_mono, q_bc_mono_reten, ad_rem_icms_reten, v_icms_mono_reten, p_red_ad_rem, mot_red_ad_rem].hash
    end
