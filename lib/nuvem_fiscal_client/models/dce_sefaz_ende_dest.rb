  ...
  class DceSefazEndeDest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NuvemFiscalClient::DceSefazEndeDest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NuvemFiscalClient::DceSefazEndeDest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @x_pais.to_s.length > 60
        invalid_properties.push('invalid value for "x_pais", the character length must be smaller than or equal to 60.')
      end

      if @x_pais.to_s.length < 2
        invalid_properties.push('invalid value for "x_pais", the character length must be greater than or equal to 2.')
      end
    ...
    def valid?
      ...
      return false if @x_pais.to_s.length > 60
      return false if @x_pais.to_s.length < 2
    ...
    def x_pais=(x_pais)
      ...
      if !x_pais.nil? && x_pais.to_s.length < 2
        fail ArgumentError, 'invalid value for "x_pais", the character length must be greater than or equal to 2.'
      end
    ...
    def hash
      [x_lgr, nro, x_cpl, x_bairro, c_mun, x_mun, uf, cep, c_pais, x_pais, fone, email].hash
    end
