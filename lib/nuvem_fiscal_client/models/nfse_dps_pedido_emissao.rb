  ...
  class NfseDpsPedidoEmissao
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NuvemFiscalClient::NfseDpsPedidoEmissao` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NuvemFiscalClient::NfseDpsPedidoEmissao`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'inf_dps')
        self.inf_dps = attributes[:'inf_dps']
      else
        self.inf_dps = nil
      end
    end
    ...
    def list_invalid_properties
      ...
      if @inf_dps.nil?
        invalid_properties.push('invalid value for "inf_dps", inf_dps cannot be nil.')
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] inf_dps Value to be assigned
    def inf_dps=(inf_dps)
      if inf_dps.nil?
        fail ArgumentError, 'inf_dps cannot be nil'
      end
    ...
    def hash
      [provedor, ambiente, referencia, inf_dps].hash
    end
