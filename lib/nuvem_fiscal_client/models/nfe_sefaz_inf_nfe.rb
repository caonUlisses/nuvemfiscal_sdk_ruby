  ...
  class NfeSefazInfNFe
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NuvemFiscalClient::NfeSefazInfNFe` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NuvemFiscalClient::NfeSefazInfNFe`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'agropecuario')
        self.agropecuario = attributes[:'agropecuario']
      end
    end
    ...
    # Custom attribute writer method with validation
    # @param [Object] pag Value to be assigned
    def pag=(pag)
      if pag.nil?
        fail ArgumentError, 'pag cannot be nil'
      end
    ...
    def hash
      [versao, id, ide, emit, avulsa, dest, retirada, entrega, aut_xml, det, total, transp, cobr, pag, inf_intermed, inf_adic, exporta, compra, cana, inf_resp_tec, inf_solic_nff, agropecuario].hash
    end
