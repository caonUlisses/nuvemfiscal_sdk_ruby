  ...
  class CteSimpSefazIdeSimp
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NuvemFiscalClient::CteSimpSefazIdeSimp` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NuvemFiscalClient::CteSimpSefazIdeSimp`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [c_uf, c_ct, cfop, nat_op, mod, serie, n_ct, dh_emi, tp_imp, tp_emis, c_dv, tp_amb, tp_cte, proc_emi, ver_proc, c_mun_env, x_mun_env, uf_env, modal, tp_serv, uf_ini, uf_fim, retira, x_det_retira, dh_cont, x_just, g_compra_gov].hash
    end
