  ...
  class NfcomSefazGSub
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NuvemFiscalClient::NfcomSefazGSub` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NuvemFiscalClient::NfcomSefazGSub`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @mot_sub.to_s.length < 2
        invalid_properties.push('invalid value for "mot_sub", the character length must be greater than or equal to 2.')
      end
    ...
    def mot_sub=(mot_sub)
      ...
      if !mot_sub.nil? && mot_sub.to_s.length < 2
        fail ArgumentError, 'invalid value for "mot_sub", the character length must be greater than or equal to 2.'
      end
