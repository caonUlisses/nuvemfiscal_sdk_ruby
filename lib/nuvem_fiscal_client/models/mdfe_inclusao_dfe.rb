  ...
  class MdfeInclusaoDfe
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NuvemFiscalClient::MdfeInclusaoDfe` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NuvemFiscalClient::MdfeInclusaoDfe`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [codigo_municipio_carrega, municipio_carrega, documentos, id, ambiente, status, autor, chave_acesso, data_evento, numero_sequencial, data_recebimento, codigo_status, motivo_status, numero_protocolo, codigo_mensagem, mensagem, tipo_evento].hash
    end
