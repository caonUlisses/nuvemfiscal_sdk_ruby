  ...
  class NfePedidoEmissaoLote
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NuvemFiscalClient::NfePedidoEmissaoLote` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NuvemFiscalClient::NfePedidoEmissaoLote`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @documentos.nil?
        invalid_properties.push('invalid value for "documentos", documentos cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @documentos.nil?
      return false if @documentos.length > 50
      return false if @documentos.length < 1
