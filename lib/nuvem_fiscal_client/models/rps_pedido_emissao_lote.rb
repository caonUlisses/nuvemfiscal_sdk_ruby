  ...
  class RpsPedidoEmissaoLote
    ...
    def self.openapi_types
      {
        :'ambiente' => :'String',
        :'referencia' => :'String',
        :'lista_rps' => :'Array<RpsPedidoEmissao>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NuvemFiscalClient::RpsPedidoEmissaoLote` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NuvemFiscalClient::RpsPedidoEmissaoLote`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'lista_rps')
        if (value = attributes[:'lista_rps']).is_a?(Array)
    ...
    def list_invalid_properties
      ...
      if !@lista_rps.nil? && @lista_rps.length > 50
        invalid_properties.push('invalid value for "lista_rps", number of items must be less than or equal to 50.')
      end
    ...
    def valid?
      ...
      return false if !@lista_rps.nil? && @lista_rps.length > 50
      true
    end
    ...
    # Custom attribute writer method with validation
    # @param [Object] lista_rps Value to be assigned
    def lista_rps=(lista_rps)
      if lista_rps.nil?
        fail ArgumentError, 'lista_rps cannot be nil'
      end

      if lista_rps.length > 50
        fail ArgumentError, 'invalid value for "lista_rps", number of items must be less than or equal to 50.'
      end
    ...
    def hash
      [ambiente, referencia, lista_rps].hash
    end
